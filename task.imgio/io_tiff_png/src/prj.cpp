...
int writeToPNG(png_byte *buf, uint32 height, uint32 width, const char *path)
{
    ...
    for (uint32 y = 0; y < height; ++y) {
        ...
        for (uint32 x = 0; x < width; ++x) {
            png_byte pixel = buf[y * height + x]; // kill x3 below
            *row++ = pixel;
            *row++ = pixel;
            *row++ = pixel;
        }
    ...
    if (tif)
    {
        ...
        for (uint32 row = 0; row < imagelength; ++row)
        {
            ...
            for (uint32 col = 0; col < scanline / 3; ++col)
            {
                ...
                result[row * imagelength + col] = gray_value;
//                result[row * imagelength + col * 3 + 1] = gray_value;
//                result[row * imagelength + col * 3 + 2] = gray_value;
