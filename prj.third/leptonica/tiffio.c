 ...
 *--------------------------------------------------------------*/
#ifndef _MSC_VER
inline intptr_t _get_osfhandle(intptr_t h)
{
    return h;
}
...
intptr_t fd_osfhandle = 0;
	...
	fd_osfhandle = (intptr_t)_get_osfhandle(fd);
    lseek(fd, 0, SEEK_SET);

    //return TIFFFdOpen(fd, "TIFFstream", modestring);
	return TIFFFdOpen(fd_osfhandle, "TIFFstream", modestring);
}
